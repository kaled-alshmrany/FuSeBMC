...
echo "Dont use this file; but runTrainTest.sh"
exit 0
echo "################### START ##########################"
echo "$0 $1 $2"
...
if [ "$#" -ne 2 ]; then
	...
	echo "ex: $0 coverage-error-call myRun"
	exit 1
fi
op=$1
#runName=$2
...
if [ "$op" == "coverage-error-call" ] || [ "$op" == "coverage-branches" ]; then
	...
	reqFilesArr=("${FUSEBMC_HOME}/FuSeBMC_engines/ESBMC/esbmc" "${FUSEBMC_HOME}/FuSeBMC_engines/ESBMC_concurrency/esbmc")
