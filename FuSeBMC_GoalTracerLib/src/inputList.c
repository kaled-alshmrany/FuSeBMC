...
void fuSeBMC_create_input_list()
{
	if(buckets_arr == NULL)
	{
		buckets_arr = fuSeBMC_alloc(buckets_total_size * sizeof(FuSeBMC_input_node_t *));
		for(int b=0; b<buckets_total_size;b++)
			buckets_arr[b] = NULL;
		buckets_arr[0] = fuSeBMC_alloc(ELEMENTS_PER_BUCKET * sizeof(FuSeBMC_input_node_t));
	}
}
	...
	*/
	...
	if(elem_idx == ELEMENTS_PER_BUCKET -1) // Bucket is full
	{
		if(bucket_idx == buckets_total_size-1) // no more free buckets
		{
			...
			// create new array.
			FuSeBMC_input_node_t ** buckets_arr_new = fuSeBMC_alloc(buckets_total_size * sizeof(FuSeBMC_input_node_t *));
			...
			{
				buckets_arr[bucket_idx] = fuSeBMC_alloc(ELEMENTS_PER_BUCKET * sizeof(FuSeBMC_input_node_t));
			}
		}
	...
	buckets_arr[bucket_idx][elem_idx].val_ptr = fuSeBMC_alloc(val_len);
	if(val_ptr != NULL)
