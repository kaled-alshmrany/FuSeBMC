...
void reach_error() { ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "relax-1.c", 91, __extension__ __PRETTY_FUNCTION__); })); }
...
int main()
{
  ...
  if(is_relaxed_prefix(pat, pat_len, a, a_len))
  {
	...
	__VERIFIER_assume(pat_len<=a_len+1);
  ...
  else if(pat_len<=a_len+1)
  {
	...
	__VERIFIER_assume(differences>1);
  }
  free(pat);
  free(a);
  return 0;
