...
TESTCOV_ERRORCALL_MEMLIMIT=8GB

TESTCOV_COVERBRANCHES_TIMELIMIT=180s
TESTCOV_COVERBRANCHES_MEMLIMIT=8GB
...
if [ -d "$OUTDIR" ]; then
	sleep 2
	dt=$(date +%d-%m-%y-%H-%M-%S)
	echo "move ${OUTDIR} to: ${dt}"
	mv "${OUTDIR}" "${OUTDIR}/../${dt}"	
	#exit 1
fi
#NEW CODE
mkdir "${OUTDIR}"
echo "${0} $@" > "${OUTDIR}/command.txt"
...
function fCleanRun() {
	if [ "$op" == "coverage-error-call" ]; then
		rm -rf ${OUTDIR}/Coverage-Error-Call/*.yml/*_*
		rm -rf ${OUTDIR}/Coverage-Error-Call/*.yml/output
		rm -f ${OUTDIR}/Coverage-Error-Call/*.yml/*.out
		rm -f ${OUTDIR}/Coverage-Error-Call/*.yml/*.gcno
		rm -f ${OUTDIR}/Coverage-Error-Call/*.yml/*.c
		rm -f ${OUTDIR}/Coverage-Error-Call/*.yml/*.gcov
		rm -f ${OUTDIR}/Coverage-Error-Call/*.yml/harness.gcda
		rm -f ${OUTDIR}/Coverage-Error-Call/*.yml/*.json
		rm -f ${OUTDIR}/Coverage-Error-Call/*.yml/*.log
		rm -f ${OUTDIR}/Coverage-Error-Call/*.yml/reduced-suite.zip
		
	elif [ "$op" == "coverage-branches" ]; then
		rm -rf ${OUTDIR}/Coverage-Branches/*.yml/*_*
		rm -rf ${OUTDIR}/Coverage-Branches/*.yml/output
		rm -f ${OUTDIR}/Coverage-Branches/*.yml/*.out
		rm -f ${OUTDIR}/Coverage-Branches/*.yml/*.gcno
		rm -f ${OUTDIR}/Coverage-Branches/*.yml/*.c
		rm -f ${OUTDIR}/Coverage-Branches/*.yml/*.gcov
		rm -f ${OUTDIR}/Coverage-Error-Call/*.yml/harness.gcda
		rm -f ${OUTDIR}/Coverage-Branches/*.yml/*.json
		rm -f ${OUTDIR}/Coverage-Branches/*.yml/*.log
		rm -f ${OUTDIR}/Coverage-Branches/*.yml/reduced-suite.zip
		
	elif [ "$op" == "unreach-call" ]; then
		rm -rf ${OUTDIR}/unreach-call/*.yml/*_*
	elif [ "$op" == "no-overflow" ]; then
		rm -rf ${OUTDIR}/no-overflow/*.yml/*_*
	elif [ "$op" == "termination" ]; then
		rm -rf ${OUTDIR}/termination/*.yml/*_*
	elif [ "$op" == "valid-memcleanup" ]; then
		rm -rf ${OUTDIR}/valid-memcleanup/*.yml/*_*
	elif [ "$op" == "valid-memsafety" ]; then
		rm -rf ${OUTDIR}/valid-memsafety/*.yml/*_*
	fi
}
...
if [ "$op" == "coverage-error-call" ]; then
	$BENCHEXEC_HOME/bin/benchexec ./FuSeBMC-testcomp.xml --tool-directory $FUSEBMC_HOME/ --no-container --numOfThreads $numOfThreads \
		--no-tmpfs --no-compress-results --limitCores -1 -d --timelimit $ERRORCALL_TIMELIMIT --walltimelimit $ERRORCALL_TIMELIMIT \
	...
	$BENCHEXEC_HOME/bin/benchexec ./TestCov.xml --tool-directory $TESTCOV_HOME/ --no-container --numOfThreads $numOfThreadsTestCov \
		--no-tmpfs  --no-compress-results --limitCores -1 -d \
...
if [ "$op" == "coverage-branches" ]; then
	$BENCHEXEC_HOME/bin/benchexec ./FuSeBMC-testcomp.xml --tool-directory $FUSEBMC_HOME/ --no-container --numOfThreads $numOfThreads \
		--no-tmpfs --no-compress-results --limitCores -1 -d --timelimit $COVERBRANCHES_TIMELIMIT --walltimelimit $COVERBRANCHES_TIMELIMIT \
